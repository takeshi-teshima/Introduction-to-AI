...
\begin{tcolorbox}[enhanced, colback=blue!2!white, colframe=blue!60!black, fonttitle=\bfseries, title={【復習とヒント】期待値・分散・共分散の性質}, drop shadow]
\begin{itemize}
    \setlength{\itemsep}{0.2em}
    \item \textbf{線形性}: $\mathbb{E}[aX + bY] = a\mathbb{E}[X] + b\mathbb{E}[Y]$
    \item \textbf{分散の定義}: $\mathbb{V}[X] = \mathbb{E}[(X - \mathbb{E}[X])^2] = \mathbb{E}[X^2] - (\mathbb{E}[X])^2$
    \item \textbf{共分散の定義}: $Cov(X,Y) = \mathbb{E}[(X - \mathbb{E}[X])(Y - \mathbb{E}[Y])]$
    \item \textbf{分散と共分散の関係}: $\mathbb{V}[X] = Cov(X,X)$
    \item \textbf{共分散の双線形性}: $Cov(aX+bY, Z) = aCov(X,Z) + bCov(Y,Z)$
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, colback=blue!2!white, colframe=blue!60!black, fonttitle=\bfseries, title={【復習とヒント】ベクトルの内積と射影}, drop shadow]
...
\end{center}
\begin{itemize}
    \item \textbf{幾何学的定義}: $\mathbf{a}^\top \mathbf{b} = \|\mathbf{a}\| \|\mathbf{b}\| \cos \theta$
    \item \textbf{直交性}: 内積が $0$ ならば、$\cos \theta = 0$ となり、2つのベクトルは直交する（$\theta = 90^\circ$）。
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, colback=blue!2!white, colframe=blue!60!black, fonttitle=\bfseries, title={【復習とヒント】行列・ベクトルの転置と微分公式}, drop shadow]
\begin{itemize}
    \setlength{\itemsep}{0.2em}
    \item \textbf{転置の積の法則}: $(\mathbf{A}\mathbf{B})^\top = \mathbf{B}^\top \mathbf{A}^\top$
    \item \textbf{スカラーの転置}: 内積はスカラー（$1 \times 1$行列）なので、転置しても値は変わらない。 \\
          $(\mathbf{x}^\top \mathbf{y})^\top = \mathbf{y}^\top \mathbf{x} = \mathbf{x}^\top \mathbf{y}$
    \item \textbf{L2ノルムの2乗}: $\|\mathbf{x}\|^2 = \mathbf{x}^\top \mathbf{x}$
