...
2クラス分類の交差エントロピー損失は、モデルの出力（ロジット）を
\(z\)、シグモイド関数を \(\sigma(z)\) とすると、次のように表される。 \[
...
\item
  ...
  \(z\) に対する損失 \(\ell(z)\) の微分
  \(\frac{\partial \ell(z)}{\partial z}\) を求め、 \[
...
\subsubsection{経験リスクの数式化（シグマを用いた書き下し）}\label{ux7d4cux9a13ux30eaux30b9ux30afux306eux6570ux5f0fux5316ux30b7ux30b0ux30deux3092ux7528ux3044ux305fux66f8ux304dux4e0bux3057}
...
\item
  \textbf{(ウ)} \(\boldsymbol{\theta} \ge \mathbf{0}\) （または
  \(\theta_1 \ge 0, \dots, \theta_d \ge 0\)）
\item
  \textbf{(エ)} \(L(\boldsymbol{\theta})\) よって全体の式は
  \(\mathop{\mathrm{Min}}_{\boldsymbol{\theta} \ge \mathbf{0}} L(\boldsymbol{\theta})\)
  となる。制約条件は \(\mathrm{Min}\) の下に記述することができる。
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  交差エントロピー損失 \(\ell(z)\) を \(z\) について偏微分する。 \[
...
\item
  ...
  である。また正則化項 \(\lambda \theta^2\) の勾配は \(2\lambda \theta\)
  である。 よって目的関数全体の勾配は
