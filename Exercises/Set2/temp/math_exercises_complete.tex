...
\begin{questionbox}[label=q:1-discrete-expectation-variance]{離散型確率分布での期待値・分散計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {不要};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-multivariate-empirical-risk]{3変数パラメータの経験リスク \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-matrix-derivative-first-order]{一階の条件の行列導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\item
  ...
  \(z\) に対する損失 \(l(z)\) の微分 \(\frac{\partial l}{\partial z}\)
  を求め、 \[
...
\item
  ...
  \item
    ...
    \(\fbox{\,カ\,}\) )
  \item
    \texttt{Step\ 2-3}: \texttt{if} 早期停止条件を満たしたら、( 目的:
    \(\fbox{\,キ\,}\) ) ( 操作: \(\fbox{\,ク\,}\) )
  \end{itemize}
\item
  \texttt{Step\ 3}: \texttt{return} \(\boldsymbol{\theta}\)
  （最適化されたパラメーター）
\end{itemize}
\end{quote}
