...
\tightlist
\item
  \textbf{スキップ可} ：解き方が分かるなら解かなくてもいい。
\item
  \textbf{確認}
  ：定義を簡単な代入問題で辿る確認や、立式などの最低限の確認。
\item
  \textbf{必須} ：講義の行間を埋める標準的な問題。
\item
  \textbf{発展} ：余裕があれば解くとよい。
\end{itemize}
...
\begin{questionbox}[label=q:1-indicator-function]{指示関数（Indicator function）の理解 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\item
  ...
  \end{enumerate}
\item
  以下では、確率変数 \(X\) は確率 \(0.2\) で \(0\)、確率 \(0.5\) で
  \(1\)、確率 \(0.3\) で \(2\) をとるとする。このとき、確率変数
  \(Y := \ind\{X \ge 1\}\)
...
\begin{questionbox}[label=q:1-discrete-expectation-variance]{離散型確率分布での期待値・分散計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:1-continuous-expectation-variance]{連続型確率分布での期待値・分散計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:1-variance-formula-proof]{共分散（Covariance）の2次形式的振る舞い \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {発展\faRocket};\hspace{2pt}}
...
\subsection{経験リスクの数式化}\label{ux7d4cux9a13ux30eaux30b9ux30afux306eux6570ux5f0fux5316}

\begin{questionbox}[label=q:2-empirical-risk-formulation]{経験リスクの立式 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:2-optimization-formulation-blank]{最適化問題の定式化：穴埋め \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\tightlist
\item
  パラメータ \(\boldsymbol{\theta}\) を調整して、目的関数
  \(L(\boldsymbol{\theta})\) を最小にする「最適なパラメータ
  \(\hat{\boldsymbol{\theta}}\)」を求める制約なし最適化問題を立式せよ。
  \[
       \mathop{\mathrm{Min}}_{(ア)} (イ)
       \] ただし、\(\boldsymbol{\theta}\) が値をとる範囲は
  \(\mathbb{R}^d\) である。
\item
  パラメータ \(\boldsymbol{\theta} = (\theta_1, \dots, \theta_d)^\top\)
  の全成分が非負（\(\theta_1 \ge 0, \dots, \theta_d \ge 0\)）であるという制約のもとで、目的関数
  \(L(\boldsymbol{\theta})\)
  の最小値を求める制約あり最適化問題を立式せよ。 \[
...
\begin{questionbox}[label=q:2-argmin-argmax-understanding]{argmin と argmax の理解 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}

最適化問題の記述において、\(\mathop{\arg~\min}\) や
\(\mathop{\arg~\max}\)
という記法が頻繁に用いられる。これらの記法の意味を確認するため、以下の値を求めよ。
ただし、ベクトル \(\boldsymbol{v}\) に対して、\(v_k\) はその第 \(k\)
成分を表すものとする。
...
\tightlist
\item
  ベクトル \(\boldsymbol{a} = (0.2, 0.7, 0.1)^\top\)
  に対して、\(\mathop{\arg~\max}_{k \in \{1, 2, 3\}} a_k\)
\item
  ベクトル \(\boldsymbol{b} = (1.5, 0.8, 2.1)^\top\) と
  \(\boldsymbol{c} = (-0.5, 1.2, -0.6)^\top\)
  に対して、\(\mathop{\arg~\min}_{k \in \{1, 2, 3\}} (b_k + c_k)\)
\end{enumerate}

\hfill 

\hyperref[a:2-argmin-argmax-understanding]{（解答・解説へ）}

\end{questionbox}

\begin{questionbox}[label=q:2-erm-parameter-function]{目的関数のパラメータ関数化 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-multivariate-gradient]{偏微分と勾配 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:2-partial-derivative-gradient]{線型単回帰モデルの最適パラメータ導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\item
  ...
  \]
\item
  上記の方程式を解くことで、最適なパラメータ \((\hat{w}, \hat{b})\)
  を求める式が \[
...
\begin{questionbox}[label=q:3-matrix-vector-multiplication-practice]{行列・ベクトルの積の計算練習 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-matrix-vector-patterns]{ベクトル・行列の積の頻出パターンと成分表示 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-inner-product-roles]{内積の役割と計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-polynomial-feature-mapping]{多項式特徴写像による線型表現 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-bag-of-words-representation]{テキストデータへの適用（Bag-of-Words）と最小二乗法 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-bivariate-gradient]{2変数関数の偏微分と勾配ベクトル \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\tightlist
\item
  関数 \(f\) の \(x\) に関する偏微分 \(\frac{\partial f}{\partial x}\)
  を求めよ。
\item
  関数 \(f\) の \(y\) に関する偏微分 \(\frac{\partial f}{\partial y}\)
  を求めよ。
\item
  求めた偏微分を縦ベクトルに並べて、関数 \(f\) の勾配ベクトル
  \(\nabla f(x, y) = \begin{pmatrix} \frac{\partial f}{\partial x} \\ \frac{\partial f}{\partial y} \end{pmatrix}\)
  を書き下せ。
\item
  点 \((x, y) = (1, 2)\) における勾配ベクトル \(\nabla f(1, 2)\)
  の値を計算せよ。
\end{enumerate}

\hfill 

\hyperref[a:3-bivariate-gradient]{（解答・解説へ）}
...
\begin{questionbox}[label=q:3-matrix-empirical-risk]{行列とベクトルによる目的関数の書き直し \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:3-quadratic-form-completing-square]{行列・ベクトルでの二次形式の展開と平方完成 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\textbf{ヒント：} スカラー \(\mathbf{b}^\top \mathbf{A} \mathbf{x}\)
...
\begin{questionbox}[label=q:3-l2-regularization-objective]{L2正則化付き目的関数の書き下し \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}

損失関数を二乗誤差、正則化項をL2ノルムの2乗とし、正則化係数を
\(\lambda = 0.01\) とする。 \(n\)
個のデータに対するL2正則化付き経験リスク最小化の目的関数
\(L(\boldsymbol{\theta})\) の式を書け。
...
\subsection{L2正則化付き線型最小二乗回帰の解}\label{l2ux6b63ux5247ux5316ux4ed8ux304dux7ddaux578bux6700ux5c0fux4e8cux4e57ux56deux5e30ux306eux89e3}

\begin{questionbox}[label=q:3-matrix-derivative-first-order]{目的関数の展開と勾配の導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}

L2正則化なしの目的関数
\(L(\boldsymbol{\theta}) = \frac{1}{2}\|\Phi\boldsymbol{\theta} - \mathbf{y}\|^2\)
の展開と勾配について考える。（次問の正則化付きの問題の準備となる。）

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{【ノルムの展開】}
  \(\|\Phi\boldsymbol{\theta} - \mathbf{y}\|^2 = (\Phi\boldsymbol{\theta} - \mathbf{y})^\top (\Phi\boldsymbol{\theta} - \mathbf{y})\)
  の右辺を展開し、目的関数が以下のように展開されることを確認せよ。 \[
...
\begin{questionbox}[label=q:3-l2-regularization-gradient]{行列による書き直しと一階の条件 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}

L2正則化付き経験リスク最小化の目的関数 \(L(\boldsymbol{\theta})\)
を考える（\(\boldsymbol{\theta}\) は \(d\) 次元のパラメーターとする）．
\[
...
で与えられることを示せ（ただし上式の逆行列は存在すると仮定する。また、\(\mathbf{I}\)
は \(d\) 次元の単位行列である）。
...
\item
  ...
  と書ける。さらには、間に単位行列 \(\mathbf{I}\) を挟んで
  \(\frac{\lambda}{2}\|\boldsymbol{\theta}\|^2 = \boldsymbol{\theta}^\top \left(\frac{\lambda}{2}\mathbf{I}\right) \boldsymbol{\theta}\)
  とみなすことができる。
\item
  目的関数全体の勾配 \(\nabla L(\boldsymbol{\theta})\) を計算し、それが
  \(\mathbf{0}\) になるという方程式（一階の条件）を立てる。
\item
  方程式を \(\boldsymbol{\theta}\)
  について解くために、\(\boldsymbol{\theta}\)
  でくくれる項をまとめ、左から逆行列を掛ける。
\end{enumerate}
...
\begin{questionbox}[label=q:3-cross-validation-indices]{K-foldとLOOCVのインデックス計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
このデータに対して、\(K=3\)
のK-fold交差検証でモデルの訓練・検証を行う。ここでは、各foldで検証用に使うデータ番号の集合を
...
とする。すなわち、第 \(k\) 回（\(k\)-th fold）の検証では、\(D_k\)
に含まれる番号のデータを検証用データとし、それ以外の番号のデータを訓練用データとする。
...
\item
  第2回の検証、すなわち \(D_2\)
  を検証用データとして用いる場合、訓練に使用されるデータ番号をすべて答えよ。
\item
  サンプルサイズが \(n=100\)
  のデータにLOOCV（一個抜き交差検証）を行う場合、モデルの学習は合計で何回実行されるか。
\end{enumerate}
...
\begin{questionbox}[label=q:4-conditional-probability-table]{同時確率表からの条件付き分布と期待値 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:4-pinball-loss-calculation]{ピンボール損失のグラフ描写の理解 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {スキップ可};\hspace{2pt}}
...
\begin{questionbox}[label=q:5-indicator-expectation]{指示関数の期待値と確率の関係 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:5-sigmoid-derivative]{シグモイド関数の微分証明 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\tightlist
\item
  \(\sigma(z) \exp(-z) = 1 - \sigma(z)\) が成り立つことを示せ。
\item
  \(\frac{d}{dz} \sigma(z) = \sigma(z)^2 \exp(-z)\) になることを示せ。
\item
  以上の結果から、\(\frac{d}{dz} \log \sigma(z) = 1 - \sigma(z)\)
  を導出せよ。
\end{enumerate}
...
\begin{questionbox}[label=q:5-cross-entropy-gradient]{交差エントロピーとチェインルールによる勾配導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}

2クラス分類の交差エントロピー損失は、正解ラベルを
\(y \in \{0, 1\}\)、モデルの出力（ロジット）を \(z\)、シグモイド関数を
\(\sigma(z)\) とすると、次のように表される。 \[
...
\def\labelenumi{\arabic{enumi}.}
\item
  まず、線型モデルなどの具体的なモデルの形を仮定せず、一般のロジット
  \(z\) に対する損失 \(\ell(z, y)\) の微分
  \(\frac{\partial \ell(z, y)}{\partial z}\) を求め、 \[
  \frac{\partial \ell(z, y)}{\partial z} = \sigma(z) - y
  \] となることを示せ。（必要に応じて前問の \(\sigma'(z)\) や
  \(\frac{d}{dz} \log \sigma(z)\) の結果を利用してよい）
\item
  次に、モデルが線型モデルであり、\(z = \boldsymbol{\theta}^\top \boldsymbol{\phi}(x)\)
  と表されるとする。このとき、チェインルール
  \(\nabla_{\boldsymbol{\theta}} \ell(z, y) = \frac{\partial \ell(z, y)}{\partial z} \cdot \nabla_{\boldsymbol{\theta}} z\)
  を用いて、パラメーター \(\boldsymbol{\theta}\) に対する勾配が \[
...
\subsection{勾配降下法と正則化}\label{ux52feux914dux964dux4e0bux6cd5ux3068ux6b63ux5247ux5316}

\begin{questionbox}[label=q:6-gradient-descent-algorithm]{勾配降下法による最適化（アルゴリズムの構成） \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}

以下の勾配降下法のアルゴリズム中の各空欄 \(\fbox{\,ア\,}\) 〜
\(\fbox{\,ク\,}\)
に当てはまる最も適切なものを、以下のそれぞれの語群から一つずつ選び、番号で答えよ。

【説明文の語群】（空欄
\(\fbox{\,ア\,}, \fbox{\,ウ\,}, \fbox{\,オ\,}, \fbox{\,キ\,}\)）

① パラメーターを更新する　② 勾配を計算する　③ 初期化する　④
反復を中断する

【数式・操作の語群】（空欄
\(\fbox{\,イ\,}, \fbox{\,エ\,}, \fbox{\,カ\,}, \fbox{\,ク\,}\)）

⑤
\(\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \mathbf{g}\)　⑥
\texttt{exit\ loop}　⑦
\(\mathbf{g} \leftarrow \nabla L(\boldsymbol{\theta})\)　⑧
\(\boldsymbol{\theta} \leftarrow \text{ランダムな値}\) （※
\(\leftarrow\) は右辺の値を左辺の変数に代入・更新する操作を表す）

【勾配降下法のアルゴリズム】

\begin{quote}
\textbf{\texttt{Algorithm:\ Gradient\ Descent}}

\begin{itemize}
\tightlist
\item
  \texttt{Step\ 1}: ( 目的: \(\fbox{\,ア\,}\) ) ( 操作:
  \(\fbox{\,イ\,}\) )
\item
  \texttt{Step\ 2}: \texttt{Loop} （以下の処理を繰り返す）

  \begin{itemize}
  \tightlist
  \item
    \texttt{Step\ 2-1}: ( 目的: \(\fbox{\,ウ\,}\) ) ( 操作:
    \(\fbox{\,エ\,}\) )
  \item
    \texttt{Step\ 2-2}: ( 目的: \(\fbox{\,オ\,}\) ) ( 操作:
    \(\fbox{\,カ\,}\) )
  \item
    \texttt{Step\ 2-3}: もし 早期停止条件を満たしたら、( 目的:
    \(\fbox{\,キ\,}\) ) ( 操作: \(\fbox{\,ク\,}\) )
  \end{itemize}
\item
  \texttt{Step\ 3}: 終了して \(\boldsymbol{\theta}\) を出力
  （最適化されたパラメーター）
\end{itemize}
\end{quote}
...
\hyperref[a:6-gradient-descent-algorithm]{（解答・解説へ）}

\end{questionbox}

\begin{questionbox}[label=q:6-weight-decay-derivation]{重み減衰（Weight Decay）の導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}

勾配法で最適化を行う場合、L2正則化は重み減衰（weight
decay）とも呼ばれる。 目的関数が経験リスク
\(\hat{R}(\boldsymbol{\theta})\) とL2正則化項の和である
\(L(\boldsymbol{\theta}) = \hat{R}(\boldsymbol{\theta}) + \lambda \|\boldsymbol{\theta}\|^2\)
で与えられるとする（\(\lambda > 0\) は正則化係数）。
...
\item
  この目的関数に対する勾配 \(\nabla L(\boldsymbol{\theta})\)
  を、\(\nabla \hat{R}(\boldsymbol{\theta})\) と \(\boldsymbol{\theta}\)
  を用いて表せ。
\item
  勾配降下法の更新式
  \(\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \nabla L(\boldsymbol{\theta})\)
  に上記で求めた勾配を代入し、「単に経験リスク \(\hat{R}\)
  を小さくする方向に進むだけでなく、現在のパラメーターを原点に向けて一定割合で縮小する」という直感的な解釈が成り立つことを、数式を用いて説明せよ。
\end{enumerate}

\hfill 

\hyperref[a:6-weight-decay-derivation]{（解答・解説へ）}

\end{questionbox}

\begin{questionbox}[label=q:6-logistic-l2-update]{ロジスティック回帰＋L2正則化の更新式と具体例計算 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}

二値分類タスクにおいて、ロジスティックモデル
\(f_\theta(x) = \sigma(\theta x)\)
を考える（バイアス項は無視し、特徴量も1次元のスカラー \(x\) とする）。
...
\tightlist
\item
  これまでの章で導出した交差エントロピーの勾配の公式を用いて、この目的関数に対する勾配降下法の更新式（パラメーター
  \(\theta\) の更新則）を書き下せ。
\item
  簡単な数値例として、\(n=2\) のデータ
  \(\{(x_1, y_1) = (1, 1), (x_2, y_2) = (-1, 0)\}\)
  を考える。現在のパラメーターが \(\theta = 0\)、学習率が
  \(\eta = 0.5\)、正則化係数が \(\lambda = 0.1\)
  のとき、1回目の更新後のパラメーター \(\theta\) の値を計算せよ。
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{ヒント（問2）：} \(\theta=0\) のとき、シグモイド関数の値は
\(\sigma(0) = 0.5\) となることに注意して計算せよ。

\hfill 

\hyperref[a:6-logistic-l2-update]{（解答・解説へ）}
...
\begin{questionbox}[label=q:6-log-softmax-gradient]{Log-Softmax 関数の勾配 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}

ベクトル \(\mathbf{z} = (z_1, \dots, z_K)^\top\)
を入力とするソフトマックス関数 \(\text{Softmax}(\mathbf{z})\)
について、以下が成り立つことを導出せよ。
...
\hyperref[a:6-log-softmax-gradient]{（解答・解説へ）}

\end{questionbox}

\begin{questionbox}[label=q:6-softmax-gradient-derivation]{ソフトマックス損失の勾配公式の導出 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {必須\faStar};\hspace{2pt}}
...
\begin{questionbox}[label=q:6-multiclass-logistic-gradient]{多クラスロジスティック回帰の勾配公式 \hfill \tikz[baseline=-0.8ex] \node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\small\bfseries, line width=0.5pt] {確認\faCheck};\hspace{2pt}}
...
\item
  ...
  \end{itemize}
\item
  問題文で与えた \(X\) の分布を用いる。\(Y = \ind\{X \ge 1\}\)
  がとり得る値は、定義より \(0\) または \(1\) です。
...
\subsubsection{経験リスクの数式化}\label{ux7d4cux9a13ux30eaux30b9ux30afux306eux6570ux5f0fux5316-1}
...
\] ここでは、経験リスク \(\hat{R}\) はパラメータ \((w,b)\)
の関数として見れば、同じ量を \(L(w,b)\) と書くこともできる。
...
\item
  \textbf{(ウ)}
  \(\boldsymbol{\theta} \in \mathbb{R}^d,\ \boldsymbol{\theta} \ge \mathbf{0}\)
  （または \(\theta_1 \ge 0, \dots, \theta_d \ge 0\)）
\item
  \textbf{(エ)} \(L(\boldsymbol{\theta})\)

  よって全体の式は \[
  \mathop{\mathrm{Min}}_{\boldsymbol{\theta} \in \mathbb{R}^d,\ \boldsymbol{\theta} \ge \mathbf{0}} L(\boldsymbol{\theta})
  \] となる。制約条件は \(\mathrm{Min}\)
  の下に記述することができる。対応する最適解を書く場合は、 \[
...
\begin{answerbox}[phantomlabel=a:2-argmin-argmax-understanding]{q:2-argmin-argmax-understanding}

\begin{itemize}
\item
  \textbf{1.} \textbf{\(2\)} \(a_k\) の値は
  \(a_1 = 0.2, a_2 = 0.7, a_3 = 0.1\) であり、\(k=2\)
  のとき最大となるため、求める値は 2 である。 \(\mathop{\arg~\max}\)
  は「対象を最大化する\textbf{入力（ここではインデックス）}」を返す記号である。\(\max_{k} a_k = 0.7\)（最大値そのもの）とは異なることに注意せよ。
\item
  \textbf{2.} \textbf{\(1\)} 各インデックスにおける \((b_k + c_k)\)
  の値は以下のようになる。
  ...
  \(\mathop{\arg~\min}\)
  は「対象を最小化する\textbf{入力（ここではインデックス）}」を返す記号である。
\end{itemize}

\hfill 

\hyperref[q:2-argmin-argmax-understanding]{（問題へ戻る）}
...
この例のように、学習における最適化の目的関数は、パラメータ \(\theta\)
の関数である。
...
\subsubsection{ベクトルと行列の計算}\label{ux30d9ux30afux30c8ux30ebux3068ux884cux5217ux306eux8a08ux7b97-1}
...
\textbf{発展補足：} この \(\Phi\) を最小二乗回帰に使う場合、正規方程式は
...
\det(\Phi^\top\Phi)=16\neq 0
\] であるため、\(\Phi^\top\Phi\)
は正則であり、逆行列が存在する。ただし、本問で求められているのは特徴ベクトル、計画行列、ラベルベクトルまでである。
...
\subsubsection{多変数関数の経験リスクと勾配}\label{ux591aux5909ux6570ux95a2ux6570ux306eux7d4cux9a13ux30eaux30b9ux30afux3068ux52feux914d-1}

\begin{answerbox}[phantomlabel=a:3-bivariate-gradient]{q:3-bivariate-gradient}
...
\hyperref[q:3-bivariate-gradient]{（問題へ戻る）}
...
\subsubsection{L2正則化付き線型最小二乗回帰の解}\label{l2ux6b63ux5247ux5316ux4ed8ux304dux7ddaux578bux6700ux5c0fux4e8cux4e57ux56deux5e30ux306eux89e3-1}
...
\item
  ...
  したがって、第2項と第3項がまとまり
  \(-2\mathbf{y}^\top \Phi \boldsymbol{\theta}\) となる。これらを全体に
  \(1/2\) 掛けたものが目的関数 \(L(\boldsymbol{\theta})\)
  となるため、与えられた式が得られる。
\item
  \(L(\boldsymbol{\theta}) = \frac{1}{2}\boldsymbol{\theta}^\top \Phi^\top \Phi \boldsymbol{\theta} - \mathbf{y}^\top \Phi \boldsymbol{\theta} + \frac{1}{2}\mathbf{y}^\top \mathbf{y}\)
  において、\(\mathbf{A} = \Phi^\top \Phi\)
...
\subsubsection{ロジスティック回帰の基礎と非線型目的関数の勾配}\label{ux30edux30b8ux30b9ux30c6ux30a3ux30c3ux30afux56deux5e30ux306eux57faux790eux3068ux975eux7ddaux578bux76eeux7684ux95a2ux6570ux306eux52feux914d-1}
...
&= \mathbb{P}(X \in A)
\end{align*} よって、常に
\(\mathbb{E}[\ind\{X \in A\}] = \mathbb{P}(X \in A)\) が成り立つ。
...
\item
  \(\sigma(z)\) の定義から計算する。 \[
       ...
       \] となる。
\item
  \(g(z) = 1 + \exp(-z)\) とおくと、\(g'(z) = -\exp(-z)\) であるから、
  \[
...
\def\labelenumi{\arabic{enumi}.}
\item
  交差エントロピー損失 \(\ell(z, y)\) を \(z\) について偏微分する。 \[
...
\subsubsection{勾配降下法と正則化}\label{ux52feux914dux964dux4e0bux6cd5ux3068ux6b63ux5247ux5316-1}

\begin{answerbox}[phantomlabel=a:6-gradient-descent-algorithm]{q:6-gradient-descent-algorithm}
...
\tightlist
\item
  \texttt{Step\ 1}　: ( 目的: ア = \textbf{③} ) \quad ( 操作: イ =
  \textbf{⑧} )
\item
  \texttt{Step\ 2-1}: ( 目的: ウ = \textbf{②} ) \quad ( 操作: エ =
  \textbf{⑦} )
\item
  \texttt{Step\ 2-2}: ( 目的: オ = \textbf{①} ) \quad ( 操作: カ =
  \textbf{⑤} )
\item
  \texttt{Step\ 2-3}: ( 目的: キ = \textbf{④} ) \quad ( 操作: ク =
  \textbf{⑥} )
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    \texttt{Step\ 2-1}: ( 目的: 勾配を計算する ) ( 操作:
    \(\mathbf{g} \leftarrow \nabla L(\boldsymbol{\theta})\) )
  \item
    \texttt{Step\ 2-2}: ( 目的: パラメーターを更新する ) ( 操作:
    \(\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \mathbf{g}\)
    )
  \item
    \texttt{Step\ 2-3}: もし 早期停止条件を満たしたら、( 目的:
    反復を中断する ) ( 操作: \texttt{exit\ loop} )
  \end{itemize}
\item
  \texttt{Step\ 3}: 終了して \(\boldsymbol{\theta}\) を出力
  （最適化されたパラメーター）
...
\hyperref[q:6-gradient-descent-algorithm]{（問題へ戻る）}

\end{answerbox}

\begin{answerbox}[phantomlabel=a:6-weight-decay-derivation]{q:6-weight-decay-derivation}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \(\nabla \|\boldsymbol{\theta}\|^2 = 2\boldsymbol{\theta}\)
  であるため、全体の勾配は
  \[ \nabla L(\boldsymbol{\theta}) = \nabla \hat{R}(\boldsymbol{\theta}) + 2\lambda \boldsymbol{\theta} \]
  となる。

  なお、正則化項を \(\frac{\lambda}{2}\|\boldsymbol{\theta}\|^2\)
  と置く流儀では、正則化項の勾配は \(\lambda\boldsymbol{\theta}\)
  になる。本問では問題文の通り \(\lambda\|\boldsymbol{\theta}\|^2\)
  と置いているため、\(2\lambda\boldsymbol{\theta}\) が現れる。
\item
  勾配降下法の更新式にこれを代入すると、 \begin{align*}
  ...
  \end{align*} となる。
  この式において、\((1 - 2\eta\lambda) \boldsymbol{\theta}\)
  の部分は現在のパラメーター \(\boldsymbol{\theta}\) を
  \(1 - 2\eta\lambda\) 倍（通常 \(0 < 1 - 2\eta\lambda < 1\)
  となるように \(\eta, \lambda\)
  が設定される）して、原点に少し近づける（縮小する）働きをしている。その上で、\(-\eta \nabla \hat{R}(\boldsymbol{\theta})\)
  によって経験リスクを減少させる方向へ更新が行われている。これが
  L2正則化が重み減衰（Weight Decay）と呼ばれる理由である。
\end{enumerate}

\hfill 

\hyperref[q:6-weight-decay-derivation]{（問題へ戻る）}

\end{answerbox}

\begin{answerbox}[phantomlabel=a:6-logistic-l2-update]{q:6-logistic-l2-update}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  前章で導出したように、1データ \((x_i, y_i)\)
  あたりの交差エントロピー損失の勾配は \((\sigma(\theta x_i) - y_i)x_i\)
  である。また本問の正則化項は \(\lambda \theta^2\) なので、その勾配は
  \(2\lambda \theta\) である。 よって目的関数全体の勾配は
...
\item
  \(\theta = 0\) のとき
  \(\sigma(\theta x_1) = \sigma(0) = 0.5\)、\(\sigma(\theta x_2) = \sigma(0) = 0.5\)
  である。 \(n=2\), \((x_1, y_1) = (1, 1)\), \((x_2, y_2) = (-1, 0)\)
  を代入して勾配を計算する。 \begin{align*}
  ...
  \end{align*} 更新後のパラメーターは
  \[ \theta \leftarrow 0 - 0.5 \times (-0.5) = 0.25 \] となる。
\end{enumerate}

\hfill 

\hyperref[q:6-logistic-l2-update]{（問題へ戻る）}
...
\begin{answerbox}[phantomlabel=a:6-log-softmax-gradient]{q:6-log-softmax-gradient}

まず、勾配ベクトルの第 \(j\) 成分である偏微分
\(\frac{\partial \log \text{Softmax}(\mathbf{z})[k]}{\partial z_j}\)
を計算する。右辺第2項の偏微分には、対数関数の微分公式
\((\log f(x))' = \frac{f'(x)}{f(x)}\)
を用いる。\(\sum_{m=1}^K \exp(z_m)\) を \(z_j\)
で微分するとどうなるかに注意して計算を進める。

\(\log \text{Softmax}(\mathbf{z})[k] = z_k - \log \sum_{m=1}^K \exp(z_m)\)
の第1項の \(z_k\) は、\(z_j\) で微分すると \(k=j\) のとき
\(1\)、\(k \neq j\) のとき \(0\) となるため、指示関数を用いて
\(\mathbf{1}\{k=j\}\) となる。

第2項は合成関数の微分則を用いて計算する。分母の和の項のうち、\(z_j\)
に依存するのは \(\exp(z_j)\) だけであることに注意すると、 \begin{align*}
...
\hyperref[q:6-log-softmax-gradient]{（問題へ戻る）}
