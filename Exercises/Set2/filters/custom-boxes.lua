    ...
    if FORMAT:match 'latex' then
      if tags then
        ...
        for tag in string.gmatch(tags, "([^,]+)") do
          tag = tag:match("^%s*(.-)%s*$")
          if tag ~= "" then
            latex_tags = latex_tags .. "\\tikz[baseline=-0.8ex] \\node[draw=white, rounded corners=2pt, inner sep=2.5pt, text=white, font=\\small\\bfseries, line width=0.5pt] {" .. tag .. "};\\hspace{2pt}"
          end
        end
