    ...
    if FORMAT:match 'latex' then
      if tags then
        ...
        for tag in string.gmatch(tags, "([^,]+)") do
          tag = tag:match("^%s*(.-)%s*$")
          if tag ~= "" then
            latex_tags = latex_tags .. "\\tcbox[enhanced, on line, boxsep=0pt, left=3pt, right=3pt, top=2pt, bottom=2pt, boxrule=0.5pt, arc=2pt, colback=white, opacityback=0, colframe=white, colupper=white, fontupper=\\small\\bfseries]{" .. tag .. "}\\hspace{2pt}"
          end
        end
